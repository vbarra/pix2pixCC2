...
class Generator(nn.Module):
    def __init__(self, opt):
        ...
        model += [pad(ks_gf//2), nn.Conv2d(input_ch, n_gf, kernel_size=ks_gf, padding=0), norm(n_gf), act]

        for _ in range(opt.n_downsample):
            model += [nn.Conv2d(n_gf, 2 * n_gf, kernel_size=ks_gd, padding=ks_gd//2, stride=2), norm(2 * n_gf), act]
            n_gf *= 2
...
class Loss(object):
    ...
    def __call__(self, D, G, input, target):
        ...
        #----------------------------------------------------------------------
        # Compute SSIM loss for the generator
        if self.opt.lambda_SSIM != 0:
            loss_G += self.opt.lambda_SSIM * ((1 - ssim(fake,target)) / 2)
